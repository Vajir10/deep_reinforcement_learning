...
from unit2.config import FrozenLakeConfig
...
class FrozenLake:
    def __init__(self,
        ...
        # ENv creation
        self.env = gym.make(env_id, map_name=map_name, is_slippery=is_slippery, render_mode=render_mode)
...
if __name__ == '__main__':
    frozenLake_obj = FrozenLake()
    frozenLake_obj.train(
    ...
    frozenLake_obj.evaluate_agent(
        max_steps=FrozenLakeConfig.MAX_STEPS,
        n_eval_episodes=FrozenLakeConfig.N_EVAL_EPISODES,
        seed = None
    )

    frozenLake_obj.record_video(out_dir=FrozenLakeConfig.VIDEO_FILENAME)
